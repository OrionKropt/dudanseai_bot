...
func NewButtonURL(text string, action ActionType, url *url.URL) Button {
	return Button{Text: text, Action: action, URL: url}
}
...
func (k *Keyboard) AddButtonURL(name string, action ActionType, url *url.URL) {
	k.markup[len(k.markup)-1] = append(k.markup[len(k.markup)-1], NewButtonURL(name, action, url))
}
